...
public class Evacuation {
    ...
    private static int maxFlow(FlowGraph graph, int from, int to) {
        ...
        while (true) {
            ...
            int[] prev = BFS(graph, from, to, edgeIndices);
            if (prev[graph.size() - 1] == Integer.MIN_VALUE){
                return flow;
            }
            flow += ReconstructPath(graph, from, to, prev, edgeIndices);
        }
    }

    //private static int ReconstructPath2(FlowGraph graph, int S, int u, int[] prev, ArrayList<Integer> eIndex) {
    private static int ReconstructPath(FlowGraph graph, int S, int u, int[] prev, int[] eIndex) {
        ...
        for (int i = 0; i < result.size(); i++) {
            graph.addFlow(result.get(i), minFlow);
        }
        return minFlow;
    }
    ...
    //bearth first search
    private static int[] BFS(FlowGraph graph, int from, int to, int[] eIndices) {
        ...
        while( !(que.isEmpty())) {
            int u = que.poll();
            for (int neigh : graph.getIds(u)) {
                Edge e = graph.getEdge(neigh);
